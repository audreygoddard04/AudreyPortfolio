...
(async () => {
  ...
  try {
    ...
    for (const route of [
      ...
      "/projects/missing",
      "/keltner/unknown",
      "/keltner/articles/missing",
      "/keltner/test-fixture",
    ])
      assert.equal((await fetch(base + route)).status, 404, route);
    const redirect = await fetch(base + "/projects/athletics", {
      redirect: "manual",
    });
    assert.equal(redirect.status, 307);
    assert.equal(redirect.headers.get("location"), "/fitness");
    ...
    assert.equal(
      (await fetch(base + "/api/send-email", { method: "OPTIONS" })).status,
      200,
    );
    ...
    assert.match(await studio.text(), /noindex/);
    await page.goto(base);
    await Promise.all([
      page.waitForNavigation(),
      page.click('footer a[href="/keltner"]'),
    ]);
    assert.equal(
      await page.$(".top-nav"),
      null,
